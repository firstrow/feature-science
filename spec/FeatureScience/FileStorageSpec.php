...
class FileStorageSpec extends ObjectBehavior
{
	...
	function it_should_increase_saves()
	{
		$this->delete('test');
		$this->increaseSaves('test'.time())->shouldBe(1);
		$this->increaseSaves('test'.time())->shouldBe(2);
		$this->increaseSaves('test'.time())->shouldBe(3);
	}
}
